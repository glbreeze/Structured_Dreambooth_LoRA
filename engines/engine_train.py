...
def train_one_epoch(accelerator: Accelerator, model: torch.nn.Module,
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        with accelerator.accumulate(model.unet):
            ...
            loss = model(pixel_values, encoder_hidden_states=encoder_hidden_states)
            accelerator.backward(loss)
            if accelerator.sync_gradients:
                params_to_clip = (
                    itertools.chain(model.unet_lora_layers.parameters(), model.text_encoder_lora_layers.parameters())
                    if args.train_text_encoder else model.unet_lora_layers.parameters())
